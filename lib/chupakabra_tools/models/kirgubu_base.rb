...
module ChupakabraTools
	module Models
		class KirguduBase < ::ActiveRecord::Base
			...
			def deletion_status_name
				::ChupakabraTools::DeletionStatus[deletion_status_id] if self.respond_to?(:deletion_status_id)
			end
			...
			def self.get_entries_for_select(controller, source, filters, options)
				...
				if source
					...
					if source < LLActiveRecord::Base || source.is_a?(LLActiveRecord::Relation)
